    ...
    for (int line_index = 0; line_index < lines->size; line_index++) {
        ...
        for (int char_index = 0; char_index < strlen(line) - 1; char_index++) {
            if (!isdigit(line[char_index]) && line[char_index] != '.') {
                ...
                for (int i = (line_index - 1 > 0 ? line_index - 1 : 0); i <= (line_index + 1 < lines->size - 1 ? line_index + 1 : lines->size - 1); i++) {
                    int found_number = 0;
                    int start_index = -1;
                    int end_index = -1;
                    for (int j = (char_index - 1 > 0 ? char_index - 1 : 0); j <= (char_index + 1 < strlen(line) - 1 ? char_index + 1 : strlen(line) - 1); j++) {
                        ...
                        if (isdigit(line2[j]) && !found_number) {
                            ...
                            start_index = j;
                            end_index = j;
                            while (isdigit(line2[start_index]) && start_index >= 0) start_index--;
                            start_index++;
                            while (isdigit(line2[end_index]) && end_index < strlen(line2) - 1) end_index++;
